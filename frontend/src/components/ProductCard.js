...
const ProductCard = ({ category, heading }) => {
    ...
    return (
        <div className='container mx-auto my-6 relative'>
            ...
            <div className="flex items-center gap-4 md:gap-6 overflow-scroll scrollbar-hide transition-all" ref={scrollElemet}>
...

                {
                    loading ? (
                        ...
                        data.map((item, index) => {
                            return (
                                <Link to={"product/" + item?._id} className="w-full min-w[280px] md:min-w-[320px] max-w[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex">
                                    <div className="bg-slate-200 h-full p-4 min-w-[120px] md:min-w-[145px]">
                                        <img src={item.productImage[0]} alt="" className='object-scale-down h-full hover:scale-110 transition-all' />
                                    </div>
