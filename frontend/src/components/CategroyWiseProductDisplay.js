...
const CategroyWiseProductDisplay = ({ category, heading }) => {
  ...
  return (
    <div className='container mx-auto my-6 relative'>
      ...
      <div className="md:gap-6 overflow-x-scroll scrollbar-hide transition-all grid grid-cols-[repeat(auto-fit,minmax(300px,320px))] justify-between">
        {
          loading ? (
            ...
            data.map((item, index) => {
              return (
                <Link to={"/product/" + item?._id} className="w-full min-w[280px] md:min-w-[320px] max-w[280px] md:max-w-[320px] bg-white rounded-sm shadow" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
